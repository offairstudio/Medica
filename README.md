...
- **כרטיסיות מקטע**: כל מקטע תוכן - במגירות, באשף ובכל מסך - יושב ב-`SectionCard` (`src/components/data/SectionCard.tsx`): הכותרת בפס עליון משלה, התוכן מתחתיה. `variant="accent"` לגוון המותג, למקטע שדורש תשומת לב.
- **ויתור על שינויים**: מגירה שיש בה טופס מזהירה לפני יציאה (`beforeClose` ב-`Sheet`) ומציגה מודל אישור - כך שהמשתמש לא מאבד מה שהזין בלחיצה על ה-X, על Esc או על הרקע.
- **פעולות בשכבות-על**: ה-X בראש המגירה או המודל הוא תמיד היציאה. הפוטר קיים רק כשיש מה להחליט: במצב צפייה הוא נושא את הפעולה בלבד (למשל "עריכה") ואין בו "סגירה" כפולה; במצב שיש בו שינויים שלא נשמרו או פעולה בעלת השלכות (עריכה, החלפה, מחיקה) הוא נושא "ביטול" ואחריו הפעולה הראשית. הפעולות תמיד בקצה השורה, הראשית אחרונה; היוצא מן הכלל הוא "חזרה" באשף, שהוא ניווט ולכן יושב בתחילת השורה.
