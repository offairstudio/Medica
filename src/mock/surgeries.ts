...
import { PLAN_PROCEDURES, plannedDays } from "./schedulePlan";
...
/**
 * מילוי היומן: כל ימי הניתוח המתוכננים (schedulePlan.ts) הופכים לניתוחים.
 * ימים שכבר נבנו ידנית למנתח מסוים נשארים כפי שהם.
 */
const manualKeys = new Set(initialSurgeries.map((s) => `${s.doctorId}|${s.date}`));

/** היומן הידני של המשתמש המחובר מכסה את השבועיים הראשונים במלואם */
const MANUAL_DOC1_UNTIL = "2026-08-09";

function minutesToTime(minutes: number): string {
  return `${String(Math.floor(minutes / 60)).padStart(2, "0")}:${String(minutes % 60).padStart(2, "0")}`;
}

const generatedSurgeries: Surgery[] = plannedDays.flatMap((day, dayIndex) => {
  if (manualKeys.has(`${day.doctorId}|${day.date}`)) return [];
  if (day.doctorId === "doc-1" && day.date <= MANUAL_DOC1_UNTIL) return [];

  return day.surgeries.map((item, k) => {
    const proc = PLAN_PROCEDURES[item.procedureIndex];
    const patient = patients[item.patientIndex % patients.length];
    return make({
      id: `sur-gen-${dayIndex}-${k}`,
      code: String(210000 + dayIndex * 7 + k),
